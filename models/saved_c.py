...
def saved_c(datastore, models):
	...
	y_pred_nn = np.round(nn_model.predict(newinDF))
	print(y_pred_nn)
	del(nn_model)
	...
	if IDboolean:
		...
		#y_pred_valid = [1 if ((nn_list[i] == 1 ) & (rf_list[i] == 1)) else 0 for i in np.range(0, len(nn_valid['Prediction'])) ]
		y_pred_combi = [1 if (nn_list[i] == rf_list[i]) else 0 for i in range(0, len(nn_list))]
